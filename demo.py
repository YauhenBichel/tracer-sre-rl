#!/usr/bin/env python3
"""Demo: runs the SRE RL environment with baseline agents.

Three agent types demonstrate the reward function's discriminative power:
  - random:    picks random actions (lower bound)
  - heuristic: investigates systematically but picks first diagnosis/remediation option
  - oracle:    investigates + uses gold-standard labels (upper bound)
"""
...
def random_agent_actions(env: SREEnvironment, rng: random.Random):
    ...
    query_types = [ActionType.QUERY_METRICS, ActionType.QUERY_LOGS, ActionType.QUERY_TRACES,
...
def main():
    ...
    parser.add_argument("--agent", choices=list(AGENTS.keys()), default=None,
                        help="Agent type (default: run all three for comparison)")
    parser.add_argument("--quiet", "-q", action="store_true",
                        help="Only show final results, not step-by-step output")
    ...
    if len(results) > 1:
        ...
        print("-" * 90)
        for r in results:
            print(f"{r['scenario'][:39]:<40} {r['agent']:<12} {r['reward']:>7.3f} "
        ...
        for agent_name in agent_names:
            ...
            avg = {k: sum(r[k] for r in agent_results) / len(agent_results)
                   for k in ("reward", "diagnosis", "efficiency", "remediation", "safety")}
            print(f"{'AVERAGE':<40} {agent_name:<12} {avg['reward']:>7.3f} "
