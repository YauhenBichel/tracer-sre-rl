...
def _format_for_grpo(trajectories: list[dict]) -> list[dict]:
    ...
    for traj in trajectories:
        ...
        completion = "\n".join(f"Action: {a['tool']}(target={a['target']})" for a in actions)
...
def _format_for_dpo(pairs_path: str) -> list[dict]:
    ...
    with open(pairs_path) as f:
        for line in f:
            ...
            chosen_actions = "\n".join(f"Action: {a['tool']}(target={a['target']})" for a in chosen.get("actions", []))
            rejected_actions = "\n".join(
...
def main() -> None:
    ...
    parser.add_argument("--data", default="training_data/trajectories.jsonl", help="Path to trajectory JSONL")
    parser.add_argument("--pairs", default="training_data/pairs.jsonl", help="Path to preference pairs JSONL (for DPO)")
    parser.add_argument("--method", choices=["grpo", "dpo"], default="grpo", help="Fine-tuning method (default: grpo)")
    parser.add_argument("--model", default="meta-llama/Llama-3.2-1B", help="Base model to fine-tune")
    parser.add_argument("--output", default="models/sre-agent", help="Output directory for fine-tuned model")
