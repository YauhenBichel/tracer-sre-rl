...
from src.generators.telemetry import TelemetryGenerator, METRIC_INTERVAL_SECONDS
...
from src.environment.state import EpisodeState, StepResult, MAX_SERVICES
...
class SREEnvironment(gym.Env):
    ...
    def __init__(self, scenario: ScenarioDefinition, seed: int | None = None, render_mode: str | None = None):
        ...
        self.remediation_options = list(r.action for r in scenario.gold_remediations) + distractor_remediations()
    ...
    def _start_episode(self, seed: int | None):
        ...
        self._telemetry = TelemetryGenerator(seed=seed).generate(perturbed)
        self._formatter = TelemetryFormatter(self._telemetry, perturbed)
    ...
    def _evaluate(self) -> StepResult:
        ...
        if s.diagnosed and s.remediated:
            reward = self._reward_calculator.calculate(
                self.scenario, {"label": s.diagnosis_label}, {"action": s.remediation_action},
                s.step_count, self.scenario.max_investigation_steps, s.action_history,
            )
            return StepResult(reward=reward, terminated=True, info={"reward_breakdown": self._reward_calculator.last_breakdown})

        if s.step_count >= self.scenario.max_investigation_steps:
            reward = 0.0
            if s.diagnosed:
                reward = self._reward_calculator.calculate(
                    self.scenario, {"label": s.diagnosis_label}, None,
                    s.step_count, self.scenario.max_investigation_steps, s.action_history,
